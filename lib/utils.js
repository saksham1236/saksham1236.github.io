...
export function helperSearch(query, node, prefix, currentLevel, maxLevel) {
    ...
    if (isRoute(node)) {
        ...
        if (titleMatch || titleDistance <= 2) {
            res.push(Object.assign(Object.assign({}, node), { items: undefined, href: nextLink }));
            parentHas = true;
        }
...
function calculateRelevance(query, title, content, headings, keywords) {
...
export function advanceSearch(query) {
    ...
    const results = chunks.flatMap((chunk) => chunk
        .map((doc) => {
        const relevanceScore = calculateRelevance(queryWords.join(" "), doc.title, doc._searchMeta.cleanContent, doc._searchMeta.headings, doc._searchMeta.keywords);
