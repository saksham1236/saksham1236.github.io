...
export function Time({ thumbnails }: TimeSliderProps) {
  ...
  return (
    ...
    >
      <Slider.Track className="h-[5px] w-full rounded-sm bg-white/30 relative">
        <Slider.Range className="bg-white absolute h-full rounded-sm will-change-[width]" />
      </Slider.Track>

      <Slider.Thumb
        aria-label="Current Time"
        className="block h-[15px] w-[15px] rounded-full border border-[#cacaca] bg-white outline-none ring-white/40 transition-opacity opacity-100 focus:opacity-100 focus:ring-4 will-change-[left]"
      />

      {/* Preview */}
      <div
        className="flex flex-col items-center absolute opacity-100 transition-opacity duration-200 will-change-[left] pointer-events-none"
        ref={previewRef}
      >
        {thumbnails ? (
          <Thumbnail.Root
