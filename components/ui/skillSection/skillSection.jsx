...
//     )
// }
export function SkillCard({ className, text, children }) {
    return (<div className={cn("h-full flex grow-0 justify-start items-center rounded-3xl md:rounded-4xl bg-primary-foreground relative contain-paint p-4 gap-4", className)}>
            {children}
            {text && <h3 className='flex justify-center text-xl md:text-2xl font-bold pr-2 md:pr-3 whitespace-nowrap'>{text}</h3>}
        </div>);
}
export function SkillCarousel({ children }) {
    ...
    return (<Carousel className="carousel" setApi={setApi} opts={{
            dragFree: true,
            loop: true
        }} plugins={[AutoScroll({
                playOnInit: true,
            ...
            <CarouselContent className="h-16 md:h-24 rounded-2xl md:rounded-4xl">
                {(Array.isArray(children) ? children : []).map((child, index) => (<CarouselItem className={cn("group h-full cursor-grab min-w-fit shrink grow-0 pl-3 md:pl-4")} key={index}>
                        {child}
                    </CarouselItem>))}
            </CarouselContent>
            <div className="sr-only flex flex-row absolute w-full justify-end px-4 bottom-[16px]">
                <div className=" flex items-center gap-2 p-2 bg-nav-noise rounded-full contain-paint shadow-lg">
                    {Array.from({ length: count }).map((_, index) => (<button key={index} onClick={() => (api === null || api === void 0 ? void 0 : api.scrollTo(index))} className={cn("size-2 rounded-full bg-primary/50 transition-all duration-150 ease-in-out", {
...
export default function SkillSection() {
    return (<div className='flex flex-col gap-6 w-full'>
            ...
            <SkillCarousel>
                ...
                <SkillCard text="Branding"><Emoji className='size-10 md:size-16' type="3d" emoji='🎨'/></SkillCard>
                <SkillCard text="Scientific Illustration"><Emoji className='size-10 md:size-16' type="3d" emoji='🧪'/></SkillCard>
                <SkillCard text="Motion Design"><Emoji className='size-10 md:size-16' type="3d" emoji='🎞️'/></SkillCard>
                <SkillCard text="Product Design"><FluentColorLightbulbFilament32 className='size-10 md:size-16'/></SkillCard>
            </SkillCarousel>
        </div>);
}
