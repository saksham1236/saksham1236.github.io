...
export default function Card({ title, subtitle, description, className, cta, link, children, dark, }) {
    ...
    return (<div className={`card lg:h-120 flex flex-col-reverse gap-4 rounded-[48px] border-1 p-6 lg:flex-row lg:gap-12 lg:p-0 contain-paint ${className}`}>
      ...
      <div className="flex aspect-auto lg:aspect-none lg:h-full flex-grow items-center justify-center carousel lg:w-1/3">
        {((Array.isArray(children))) ? (<Carousel className="carousel aspect-3/2 lg:aspect-auto lg:h-full" setApi={setApi} plugins={[]}>
            <CarouselContent className="h-full rounded-4xl">
              {(Array.isArray(children) ? children : []).map((child, index) => (<CarouselItem className={cn("transition-all duration-700 ease h-full cursor-grab", index == 0 ? "lg:basis-1/1" : "lg:basis-4/5")} key={index}>
          ...
          </Carousel>) : (<div className="h-full w-full rounded-4xl contain-paint">
            {children}
          </div>)}
      </div>
    </div>);
