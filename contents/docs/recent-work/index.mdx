---
title: My Work
description: A showcase of my work, including UX/UI Design projects, scientific illustrations, and development work.
---
## Design Work
<CardGrid>
    <Card
    variant = "image"
    image = "/images/cdw-canada/thumb-bg.png"
    title="CDW Canada"
    description="Delivered high-impact web designs and automated internal tools to boost team efficiency."
    href="mailto:s9tandon@uwaterloo.ca"
    subtitle="UX Design, Web Dev, IT Solutions"
    cta={true}
    ctaContent="Request Work"
  />
    <Card
    variant = "image"
    image = "/images/bb-learning/thumb.png"
    title="Bounce Back Learning App"
    description="Bounce Back Coin Learning platform, an LMS where users earn crypto via education, manage their crypto portfolios via a wallet dashboard, and access discussion boards/messaging."
    href="/docs/recent-work/bounce-back-learning"
    subtitle="Web Design, UX/UI Design"
    cta={true}
  />
  <Card
    variant = "image"
    image = "/images/lynk/thumb.jpg"
    title="Lynk"
    href="/docs/recent-work/lynk"
    description="Lynk is a platform designed to combat social isolation by connecting individuals with local activities and communities."
    cta={true}
    subtitle="Product Design, UX/UI Design"
  />
  <Card
    variant = "image"
    image = "/images/bb-digital-branding/thumb.png"
    title="Bounce Back Digital Branding"
    description="Showcasing, branding guidelines, and marketing assets for Bounce Back Digital a Digital Agency/Fintech Startup."
    href="/docs/recent-work/bounce-back-digital-branding"
    cta={true}
    subtitle="Branding"
  />
  <Card
    variant = "image"
    image = "/images/bb-coin-branding/header.png"

    title="Bounce Back Coin Branding"
    description="Bounce Back Coin is a learning platform that lets users earn cryptocurrency by participating in educational activities. Our goal is to make learning fun and rewarding."
    href="/docs/recent-work/bounce-back-coin-branding"
    cta={true}
    subtitle="Web Design, UX/UI Design, Branding, Animation"
  />
  <Card
    variant = "image"
    image = "/images/mind-games/hero.jpg"
    title="Mind Games"
    href="/docs/recent-work/mind-games"
    description ="We aimed to create a game library subscription service focused on mental health to deal with the increased levels of mental health disorders during the COVID-19 pandemic."
    cta = {true}
  subtitle="UX/UI Design, Branding, Product Design"
  />
  <Card
    variant = "image"
    image = "/images/young-funds/thumb.png"
    title="Young Funds Website Redesign"
    href="https://www.behance.net/gallery/90910587/Young-Funds-website-redesign"
    description ="Young Funds Program is a initiative that aims to educate students about investing from an early age. The website redesign was part of a school project."
    cta = {true}
    external = {true}
    ctaContent="View on Behance"
  subtitle="UX/UI Design, Web Design"
  />
  <Card
    variant = "image"
    image = "/images/ican/thumb.png"
    title="iCan App UX/UI Design"
    href="https://www.behance.net/gallery/90909937/iCan-UXUI-Design-Project"
    description ="iCan is a UX/UI Design project that aims at helping people coming to Canada by providing them with many services in a single application."
    cta = {true}
    external = {true}
    ctaContent="View on Behance"
    subtitle="UX/UI Design, Product Design"
  />
    <Card
    variant = "image"
    image = "/images/claimos/thumb.png"
    title="Claimos UI Redesign"
    description="Redesigned the marketing dashboard, analytics dashboard, and file upload pages to improve clarity, navigation, and overall user experience."
    subtitle="UX/UI Design"
  >
  <Button icon = {<LineMdHazardLightsOffLoop className = "size-6 text-red-400"/>}>Coming Soon</Button>
  </Card>
</CardGrid>
---
## Dev Work
<CardGrid>
    <Card
    icon = "figma"
    title="Re-Layer Figma Plugin"
    subtitle = "Opens in Figma"
    description="A Figma plugin that automates layer renaming with tokens and conditional rules."
    href="https://www.figma.com/community/plugin/1499494471654319987/re-layer"
    cta={true}
    external = {true}
  />
  <Card
    icon = "github"
    title="GlassDoor Salary Data Analysis"
    subtitle = "Opens on Github"
    description="I analyzed the Glassdoor tech jobs dataset from Kaggle to identify the highest-paying skill for data scientists among Python, R, Spark, AWS, and Excel."
    href="https://github.com/saksham1236/Data-Salary-Analysis"
    cta={true}
    external = {true}
  />
  <Card
    icon = "github"
    title="BinThere"
    subtitle = "Opens on Github"
    description=" Web-based app designed to make waste disposal accessible and pleasant. Designed for those who want to do their part for the environment but don’t have the time, ability, or motivation to do so."
    href="https://github.com/saksham1236/industry-project-st-fork"
    cta={true}
    external = {true}
  />
  <Card
    icon = "github"
    title="Layoff Warning System"
    subtitle = "Opens on Github"
    description="This web app scrapes publicly accessible WARN notices from government websites, logging them in a database for access via a React-based frontend."
    href="https://github.com/saksham1236/saksham-tandon-capstone-lws-front-end"
    cta={true}
    external = {true}
  />
  <Card
    icon = "github"
    title= "Instock"
    subtitle = "Opens on Github"
    description="Collaboratively contributed to the development of Instock, a BrainStation student project, where responsibilities included building the React front end, Node.js and Express.js backend, and integrating a MySQL database using Knex.js."
    href="https://github.com/saksham1236/brnstn_instock-mp2-frontend"
    cta={true}
    external = {true}
  />
</CardGrid>

---
## Scientific Illustration Work
<CardGrid>
  <Card
    variant = "image"
    image = "/images/more-design-work/rydberg-atom-thumb.png"
    subtitle = "Opens on Behance"
    title="Rydberg Atom Quantum Simulator"
    href="https://www.behance.net/gallery/202608739/Rydberg-Atom-Quantum-Simulator-Diagrams"
    cta={true}
    external = {true}
  />
  <Card
    variant = "image"
    image = "/images/more-design-work/tear-film-layer-cover.png"
    subtitle = "Opens on The Cell Press Website"
    title=" Journal Cover - Biophysical properties of tear film lipid layer II. Polymorphism of FAHFA"
    href="https://www.cell.com/biophysj/issue?pii=S0006-3495(21)X0003-3#fullCover"
    cta={true}
    external = {true}
  />
  <Card
    variant = "image"
    image = "/images/more-design-work/acs-nano-cover-sub.png"
    subtitle = "Opens on Behnance"
    title="Journal Cover Submission - Airborne Transmission of COVID-19"
    href="https://www.behance.net/gallery/111582107/ACS-Nano-Cover-submission"
    cta={true}
    external = {true}
  />
  <Card
    variant = "image"
    image = "/images/more-design-work/preview-color-blindness-animation.gif"
    subtitle = "Opens on Youtube"
    title="Color Blindness Animation"
    href="https://www.youtube.com/watch?v=iBTqyTwAWig&t=2s"
    cta={true}
    ctaContent="View on Youtube"
    external = {true}
  />
  <Card
    variant = "image"
    image = "/images/more-design-work/atomic-force-microscopy.png"
    subtitle = "Opens on Biohysical"
    title="Atomic Force Microscopy Setup"
    href="https://www.behance.net/gallery/113015787/Scientific-Illustration-University-of-Hawaii-intern"
    cta={true}
    external = {true}
  />
  <Card
    variant = "image"
    image = "/images/more-design-work/atomic-force-microscopy-cover.png"
    subtitle = "Opens on Cell Press Website"
    title="Journal Cover - Atomic Force Microscopy Imaging of Adsorbed Pulmonary Surfactant Films"
    href="https://www.cell.com/biophysj/issue?pii=S0006-3495(19)X0017-X#fullCover"
    cta={true}
    external = {true}
  />
</CardGrid>
---
